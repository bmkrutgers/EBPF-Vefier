...
#define bpf_test_trace(x,y) verbose("PASSED %s in BPF instruction", x)
	...
	if (opcode == BPF_END || opcode == BPF_NEG) {
		if (opcode == BPF_NEG) {
            ...
            else
            {
                    ...
                    bpf_test_trace("BPF_NEG", insn->code);
			...
			if (insn->src_reg != BPF_REG_0 || insn->off != 0 ||
				...
				return -EINVAL;
			}//else clause to bpf_check
            
            else
            {
                
                    bpf_test_trace("BPF_END", insn->code);
		...
		if (BPF_SRC(insn->code) == BPF_X) {
        ...
        else
            {
                    ...
                    bpf_test_trace("BPF_MOV", insn->code);
            ...
            else
            {
                    ...
                    bpf_test_trace("BPF_MOV", insn->src_reg);
